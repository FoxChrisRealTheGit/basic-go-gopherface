...
func DisplayPostPreview(w http.ResponseWriter, r *http.Request, p *PostForm){
	...
	post := socialmedia.NewPost("anonymouse gopher", socialmedia.MoodState(moodState), p.Fields["caption"], p.Fields["messageBody"], "", "", "", nil)
	RenderTemplate(w, "./templates/socialmediapsot.html", post)
}
...
func ValidatePostForm(w http.ResponseWriter, r *http.Request, p *PostForm){
	...
	if r.FormValue("captian") == ""{
		p.Errors["messageBodyError"] = "The caption field is required."
	}
...
func init(){
	...
	MoodStates["Shy"] = int(socialmedia.MoodStateBored)
